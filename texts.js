...
const addText = ({ text, question }) => {
  let score = calculateComplexityScore(text);
  let speed = interpolate(score);
  texts.push({ text, question, score, speed });
};
...
addText({
  ...
  question:
    "A financial accounting system is only intended for using inside of an organization.",
});
...
addText({
  ...
  question: "Coral reefs consist of living components.",
});
...
addText({
  ...
  question:
    "A patient who is not responding to tradition therapy is an excellent candidate for music therapy.",
});
...
addText({
  ...
  question: "An ecosystem consists of animals",
});
...
addText({
  ...
  question:
    "Replacing incandescent lights with fluorescents can result in a savings of more than 50% on your monthly lighting costs.",
});
...
addText({
  ...
  question: "The first bicycle was invented in France.",
});
...
addText({
  ...
  question: "Eating disorder is a rare disease in the USA.",
});
...
addText({
  ...
  question:
    "There are two types of diabetes, insulin dependent and non-insulin-dependent.",
});
...
function qualityMetrics() {
  ...
  const stats = (group) => ({
    ...
    mean: group.map((entry) => entry.score).reduce((a, b) => a + b) / group.length,
  });
